...
def _create_session_caches(
    ...
    """Create the session context cache. Logs but does not raise on failure."""
    try:
        cache_manager.create_cache("normal", system_prompt, api_tools)
    except Exception as e:
        _log.warning("cache_create_failed key=normal error=%s", e)
...
def _refresh_caches(
    ...
    """Rebuild the session cache after a server-side expiry.

    Best-effort: _create_session_caches swallows failures, so a partial
    failure leaves the session running uncached until the next refresh.
    """
...
def _try_recover_cache_expiry(
    ...
    *,
    ...
    """Detect a Vertex cache-expired error and refresh the cache in place.

    Returns True if the caller should retry (cache was refreshed), False
    ...
    _refresh_caches(cache_manager, system_prompt, api_tools)
    return True
