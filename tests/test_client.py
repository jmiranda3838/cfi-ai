...
def test_detect_repetition_consecutive_suffix():
    ...
    block = "x" * _REPEAT_BLOCK_SIZE
    text = "preamble " + block + block
    assert _is_repeated_suffix(text)


def test_detect_repetition_false_short():
    """Text shorter than two full blocks cannot trigger detection."""
    # _is_repeated_suffix needs at least 2 * _REPEAT_BLOCK_SIZE chars to have
    # a preceding block to compare against.
    block = "a" * (_REPEAT_BLOCK_SIZE - 1)
    text = "x" + block + block  # one char short of two full blocks
...
def test_detect_repetition_false_unique():
    ...
    text = "".join(chr(65 + (i % 26)) for i in range(_REPEAT_MIN_TEXT_LENGTH + _REPEAT_BLOCK_SIZE))
    assert not _is_repeated_suffix(text)


def test_detect_repetition_false_non_consecutive():
    """Same block appears twice but separated by different content — no trigger."""
    block = "R" * _REPEAT_BLOCK_SIZE
    separator = "S" * _REPEAT_BLOCK_SIZE
...
def test_text_chunks_stops_on_repetition():
    ...
    block = "Z" * _REPEAT_BLOCK_SIZE
    # Build enough preamble to pass the minimum-text threshold, then repeat.
    preamble = "A" * (_REPEAT_MIN_TEXT_LENGTH - _REPEAT_BLOCK_SIZE)
