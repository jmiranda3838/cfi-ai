...
- The four clinical maps (`/intake`, `/session`, `/compliance`, `/tp-review`) all include a **Resolve Payer** phase up front (Phase 0 in `compliance.py` / `tp_review.py`, Phase 1 in `intake.py` / `session.py`) — load the client profile, read the Payer field, map to a slug, and call `load_payer_rules`. Subsequent phases (load records / analyze / write) operate against the loaded payer rules so payer-specific CPT/modifier/authorization/assessment logic stays out of the generic prompts. See `### Payer rules` below.
...
- The `load_payer_rules` tool (`tools/load_payer_rules.py`) takes one required `payer` arg with `enum: list(VALID_PAYERS)` and returns `PAYER_RULES[payer]`. Non-mutating. The map prompts instruct the LLM to call it exactly once per workflow during the Resolve Payer phase.
- Slug-mapping convention (encoded in every clinical map's Resolve Payer phase): `"Optum EWS/EAP"` or `"Optum EAP"` → `optum-eap`; `"Aetna"` → `aetna`; `"Evernorth"` → `evernorth`. If the Payer field on the client profile is missing or unclear, the map prompts require a single `interview` call rather than guessing.
